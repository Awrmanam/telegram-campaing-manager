...
async def main() -> None:
    ...
    clients = ClientManager(settings.telegram_api_id, settings.telegram_api_hash, settings.session_directory)
    delivery = DeliveryService(sessions, clients, settings.min_send_delay_seconds, settings.max_send_delay_seconds)
    scheduler = CampaignScheduler(sessions, delivery, settings.timezone, settings.catch_up_missed_runs)
    bot, dispatcher = Bot(settings.bot_token), Dispatcher()
    dispatcher.update.outer_middleware(AdminMiddleware(settings.admin_ids))
    dispatcher.include_router(build_router(sessions))
    try:
