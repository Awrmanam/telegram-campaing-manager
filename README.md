...
فعال‌سازی و اجرای دستی فقط وقتی مجاز است که حساب، حداقل یک پیام و حداقل یک مقصد همگی فعال و متعلق به همان حساب باشند. حذف یا غیرفعال‌کردن آخرین مقصد، کمپین فعال را خودکار متوقف می‌کند. FloodWait اجرای زمان‌بندی‌شده cursor و پیام را برای ادامه نگه می‌دارد؛ FloodWait اجرای دستی در `manual_retry_at` و cursor مستقل ثبت می‌شود و هرگز `next_run_at` دوره‌ای را بازنویسی نمی‌کند.
